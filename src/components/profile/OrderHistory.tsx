...
const OrderHistory: React.FC<OrderHistoryProps> = ({ orders }) => {
  ...
  return (
    <div className="space-y-6">
      {orders.map((order) => (
        <div key={order._id} className="border border-black/5 p-6 rounded-sm space-y-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              <Package size={16} className="text-luxury-red" />
              <span className="text-[10px] font-bold tracking-widest uppercase">ORDER #{order._id.slice(-8)}</span>
            </div>
          ...
          <div className="flex justify-between items-end border-t border-black/5 pt-4">
            ...
            <div className="text-sm font-bold text-luxury-red">
              {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(order.totals.total)}
            </div>
          </div>
        </div>
      ))}
